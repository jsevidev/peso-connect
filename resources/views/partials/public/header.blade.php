...
@endphp
<header id="public-header" class="public-header" style="background-color: #fff;display: flex;flex-direction: row;grid-column-gap: 0px;align-items: center;justify-content: center;height: 80px;position: relative;flex-shrink: 0;">
  <a href="{{ route('home') }}" class="public-brand" data-turbo-frame="public-content" style="display: flex;flex-direction: row;grid-column-gap: 12px;align-items: center;justify-content: flex-start;width: min-content;position: relative;flex-shrink: 0;margin: 0px auto 0px 0px;text-decoration: none;">
    <div style="overflow: hidden;border-radius: 20px;display: flex;flex-direction: column;row-gap: 0px;align-items: start;justify-content: flex-start;width: 40px;height: 40px;position: relative;flex-shrink: 0;"><img src="{{ asset('assets/img/peso-logo.png') }}" alt="PESO seal logo" style="inset: 0;width: 100%;height: 100%;position: absolute;object-fit: cover;"></div>
  ...
  <nav id="public-nav" class="public-nav" style="display: flex;flex-direction: row;grid-column-gap: 8px;align-items: center;justify-content: flex-start;width: min-content;position: relative;flex-shrink: 0;margin: 0px auto 0px 0px;">
    @foreach ($navItems as $item)
      ...
      @endphp
      <a href="{{ route($item['route']) }}" data-turbo-frame="public-content" style="{{ $activeStyle }}display: flex;flex-direction: column;row-gap: 0px;align-items: start;justify-content: flex-start;width: min-content;position: relative;flex-shrink: 0;padding: 10px 16px;text-decoration: none;"><span class="text" style="display: inline;text-align: left;font-size: 14px;font-family: Inter, system-ui, sans-serif;font-weight: {{ $fontWeight }};font-stretch: 100%;color: #1b3a6b;width: max-content;position: relative;flex-shrink: 0;">{{ $item['label'] }}</span></a>
    @endforeach
  </nav>
  <a href="{{ route('enlistment') }}" class="public-header__enlist--desktop" data-turbo-frame="public-content" style="border-radius: 8px;background-color: #1b3a6b;display: flex;flex-direction: row;grid-column-gap: 0px;align-items: center;justify-content: center;width: min-content;position: relative;flex-shrink: 0;padding: 10px 24px;text-decoration: none;cursor: pointer;"><span class="text" style="display: inline;text-align: center;font-size: 14px;font-family: Inter, system-ui, sans-serif;font-weight: 600;font-stretch: 100%;color: #fff;width: max-content;position: relative;flex-shrink: 0;">Enlist Now</span></a>
</header>
